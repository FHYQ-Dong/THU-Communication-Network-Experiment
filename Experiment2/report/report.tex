...
\begin{enumerate}[label=(\arabic*)]
    ...
    \textbf{实验结果：}在生成高斯噪声时，应选择星座图的理论符号能量。信道噪声的方差$\sigma^2$是信道本身的统计特性，定义时通常相对于信号的统计平均能量（即期望值）。如果使用单次随机序列的平均能量，会导致噪声功率随着发送数据的具体内容而波动，这不符合实际信道的物理模型。理论符号能量代表了长期的统计平均，能保证信噪比定义的准确性和一致性。

    \item 比较原始比特序列和解调后的比特序列。分别统计不同信噪比、不同仿真次数$N$下的符号差错个数和比特差错个数。

    \textbf{实验结果：}不同信噪比和仿真次数下的符号差错数与比特差错数统计如表 \ref{tab:exp1_symbol_res} 和表 \ref{tab:exp1_bit_res} 所示。
    
    \begin{table}[h]
        \centering
        \caption{实电平传输：符号差错个数统计 (M=2)}
        \label{tab:exp1_symbol_res}
        \begin{tabular}{|c|c|c|c|}
            \hline
            \multirow{2}{*}{SNR (dB)} & \multicolumn{3}{c|}{仿真次数 $N$}\\
            \cline{2-4}
             & $10^4$ & $10^5$ & $10^6$ \\ 
            \hline
            2 & 756 & 7885 & 78690 \\
    ...
    \textbf{实验结果：}符号率($P_s$)和误比特率($P_b$)随信噪比的变化曲线如图 \ref{fig:exp1_ser} 和图 \ref{fig:exp1_ber} 所示。
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \textbf{实验结果：}$M=16$ QAM调制在SNR=15dB和20dB下的接收信号星座图如图 \ref{fig:exp2_constellation} 所示。可见信噪比越高，接收符号点越集中于理论星座点附近。
    ...
    \textbf{实验结果：}差错个数统计如表 \ref{tab:exp2_symbol_res} 和表 \ref{tab:exp2_bit_res} 所示。
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{|c|c|c|c|c|c|c|}
            \hline
            \multirow{2}{*}{SNR} & \multicolumn{3}{c|}{$M=4$ ($N$)} & \multicolumn{3}{c|}{$M=16$ ($N$)} \\
            \cline{2-7}
    ...
    \textbf{实验结果：}误符号率和误比特率随信噪比变化曲线如图 \ref{fig:exp2_curves} 所示。
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \textbf{实验结果：}等效信噪比随波形持续时间$T$的变化如图 \ref{fig:exp3_snr_T} 所示。
...
1. 验证了高斯白噪声下不同调制方式（M-PAM, M-QAM）的误码率性能，仿真结果与理论公式高度吻合。
2. 观察了星座图随信噪比的变化，直观感受了噪声对信号判决的影响。
3. 在波形信道实验中，体会了匹配滤波器在抗噪性能上的优势，理解了信号能量与时间$T$的关系。
