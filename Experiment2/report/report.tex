...
\begin{enumerate}[label=(\arabic*)]
    ...
    \textbf{分析：}观察仿真结果可知，仿真曲线与理论曲线基本重合。这表明仿真模型正确地反映了高斯白噪声信道下的传输特性。随着信噪比的增加，误码率呈瀑布状迅速下降。由于低SNR时误符号落在非相邻符号的概率增大，故此时实际误比特率比按照格雷规则计算的理论值要大。
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
	...
	\item 同时画出理论值对应的曲线，观察仿真结果是否与理论值相符，分析原因。 \\ 
    \textbf{分析：}仿真结果与理论值吻合。对比M=4和M=16，可以看出在相同信噪比下，M=16的误码率显著高于M=4。这是因为M=16的星座点更密集，最小欧氏距离更小，因此更容易受噪声干扰发生误判。由于低SNR时误符号落在非相邻符号的概率增大，故此时实际误比特率比按照格雷规则计算的理论值要大。
\end{enumerate}
